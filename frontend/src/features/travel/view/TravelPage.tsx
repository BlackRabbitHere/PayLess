...
export function TravelPage() {
  const { travel, setTravel, submit, loading, reset } = useOptimization()
  ...
  return <main className="page"><div className="page-title-row"><div><span className="eyebrow">SAME TRIP. A BETTER WAY TO PAY.</span><h1>Find your travel route.</h1><p>Compare booking fares, offers, and your wallet in one place.</p></div><Plane size={32} /></div>
    ...
    <section className="panel live-search"><form onSubmit={async event => { event.preventDefault(); if (await submit('travel')) navigate('/results') }}>
      <div className="live-travel-fields">
        <label>From<select aria-label="From" value={travel.origin} onChange={event => setTravel({ ...travel, origin: event.target.value })}>{travelCities.map(city => <option key={city}>{city}</option>)}</select></label>
        <label>To<select aria-label="To" value={travel.destination} onChange={event => setTravel({ ...travel, destination: event.target.value })}>{travelCities.map(city => <option key={city}>{city}</option>)}</select></label>
        <label>Departure<input aria-label="Departure" type="date" min={today()} required value={travel.departureDate} onChange={event => setTravel({ ...travel, departureDate: event.target.value })} /></label>
        <label>Passengers<select aria-label="Passengers" value={travel.passengers} onChange={event => setTravel({ ...travel, passengers: Number(event.target.value) })}>{[1, 2, 3, 4, 5, 6].map(n => <option value={n} key={n}>{n}</option>)}</select></label>
