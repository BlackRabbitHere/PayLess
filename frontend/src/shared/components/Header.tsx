...
export function Header() {
  ...
  return <>
    ...
    {profile && <Modal title="Your demo space" onClose={() => setProfile(false)}><div className="profile-summary"><span className="profile-avatar">AK</span><div><h3>Alex Kumar</h3><p>Demo profile · no sign-in required</p></div></div><div className="notice notice-primary"><ShieldCheck size={20} /><p>Only payment-product names and demo searches are saved in this browser. No card credentials are collected.</p></div><div className="stack mt-6"><button className="button button-primary" onClick={() => { setProfile(false); navigate('/wallet') }}><Wallet size={17} />Manage My Wallet</button><button className="button button-secondary" onClick={() => { reset(); optimization.reset(); setProfile(false); navigate('/') }}><RotateCcw size={16} />Reset all demo data</button></div></Modal>}
  </>
}

export function Footer() {
  return <footer className="app-footer"><Brand small /><span>Better decisions. Every payment.</span><span className="footer-disclaimer"><ShieldCheck size={14} />Explore routes · Pay with the provider</span><Link to="/system" className="text-button">Service status</Link></footer>
}
