import { initial, readState, storageKey, type SavedState } from '../../features/optimization/model/state'
...
export function PrototypeProvider({ children }: { children: ReactNode }) {
  ...
  function update(updateState: (current: SavedState) => SavedState) {
    setState(current => {
      ...
      try { localStorage.setItem(storageKey, JSON.stringify(next)) } catch { /* In-memory interactions remain available. */ }
      return next
    })
  }
  ...
  return <PrototypeContext.Provider value={{ ...state,
    ...
    reset: () => { update(() => initial()); toast('Demo reset. Your starting wallet and searches are restored.') }, toast,
  }}>
    {children}
